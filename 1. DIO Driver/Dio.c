 ...
 * @details
 * Hàm này đọc trạng thái logic của một chân GPIO (DIO Channel) 
 ...
 *
 * @param[in] ChannelId
 *            ID của kênh DIO cần đọc (xác định bởi macro DIO_CHANNEL)
 *
 * @return Dio_LevelType
 *         Trạng thái logic của kênh được chỉ định:
 ...
 * @details
 * Hàm này ghi trạng thái logic (HIGH hoặc LOW) cho một chân GPIO 
 * (DIO Channel) trên vi điều khiển STM32F103. Hàm xác định cổng 
 * và chân GPIO dựa trên ChannelId và sau đó ghi mức logic được chỉ định.
 *
 * @param[in] ChannelId
 *            ID của kênh DIO cần ghi (xác định bởi macro DIO_CHANNEL)
 * 
 * @param[in] Level
 *            Trạng thái logic cần ghi vào kênh DIO (STD_HIGH hoặc STD_LOW)
 **********************************************************/
 ...
 * @details
 * Hàm này đọc trạng thái logic của tất cả các chân GPIO 
 ...
 *
 * @param[in] PortId
 *            ID của cổng DIO cần đọc (DIO_PORT_A, DIO_PORT_B, ...)
 *
 ...
 * @details
 * Hàm này ghi một giá trị trạng thái logic (16-bit) cho 
 ...
 *
 * @param[in] PortId
 *            ID của cổng DIO cần ghi (DIO_PORT_A, DIO_PORT_B, ...)
 *
 * @param[in] Level
 *            Giá trị trạng thái logic cần ghi cho toàn bộ cổng GPIO
 **********************************************************/
 ...
 * @details
 * Hàm này đọc trạng thái logic của một nhóm chân GPIO 
 ...
 *
 * @param[in] GroupIdPtr
 *            Con trỏ đến cấu trúc nhóm DIO (chứa thông tin cổng,
 *            mask và offset)
 *
 * @return Dio_PortLevelType
 ...
 * @details
 * Hàm này ghi một giá trị trạng thái logic vào một nhóm chân GPIO
 ...
 *
 * @param[in] Level
 *            Giá trị trạng thái logic cần ghi cho nhóm kênh DIO
 **********************************************************/
void Dio_WriteChannelGroup(const Dio_ChannelGroupType* GroupIdPtr, Dio_PortLevelType Level) {
    ...
    portData |= ((Level << GroupIdPtr->offset) & GroupIdPtr->mask);  /* Ghi giá trị mới vào nhóm chân */
 ...
 * @details Hàm này lật trạng thái logic của một chân GPIO (HIGH thành LOW hoặc ngược lại).
 * 
